...
def Q5(vdo_df):
    ...
    comedy_id1 = 23
    comedy_id2 = 34
    sports_df = vdo_df[vdo_df['category_id'] == sports_id].groupby('trending_date')['views'].sum()
    comedy_df = vdo_df[vdo_df['category_id'] == (comedy_id1 or comedy_id2)].groupby('trending_date')['views'].sum()
    combined_df = pd.DataFrame({
        'sports_views': sports_df,
        'comedy_views': comedy_df
    }).fillna(0)
    sports_morethan_comedy = (combined_df['sports_views'] > combined_df['comedy_views']).sum()
